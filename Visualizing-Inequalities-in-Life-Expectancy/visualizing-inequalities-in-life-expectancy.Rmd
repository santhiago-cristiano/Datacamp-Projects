...
```{r}
# Loading data
life_expectancy <- tibble(read.csv("datasets/UNdata.csv"))
...
ggplot(subdata) +
  aes(x = Male, y = Female, label = Country.or.Area) +
  geom_point(
    colour = "white",
    fill = "chartreuse3",
    shape = 21,
  ...
  geom_text(top_male, mapping = aes(label = Country.or.Area)) +
  geom_text(top_female, mapping = aes(label = Country.or.Area)) +
  theme_bw()
```
...
ggplot(subdata2) +
  ...
  labs(
    title = "Life Expectancy at Birth by Country in Years",
...
top <- subdata2 %>%
  ...
  select(-Source, -Unit, - Value.Footnotes) %>% 
  head(3)
...
ggplot(subdata2, guide = FALSE) +
  ...
  geom_text(top, mapping = aes(label = Country.or.Area)) +
  geom_text(bottom, mapping = aes(label = Country.or.Area)) +
  theme_bw()
```
